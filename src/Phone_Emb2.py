# -*- coding: utf-8 -*-
"""
Created on Tue Jul  5 13:50:46 2016

@author: ambroise


GOAL: Phone_Emb2 get phone from the previous one AND the next one. Order deos matter (concatanation)
      The transform of the left and right context in the firs hidden layer is the same in order to get the same embedding

Input: output of GetPhones_dictio

Output: W model, embedding
...
class HiddenLayer(object):
    def __init__(self, rng, input, n_in, n_out, W=None, b=None,
        ...
        lin_output_left = T.dot(input[0], self.W) + self.b
        self.output_left = (
            lin_output_left if activation is None
            else activation(lin_output_left)
        )

        lin_output_right = T.dot(input[1], self.W) + self.b
        self.output_right = (
            lin_output_right if activation is None
            else activation(lin_output_right)
        )
        # parameters of the model
        self.params = [self.W, self.b]
...
#        )
        ...
        self.logRegressionLayer = LogisticRegression(
            input=T.concatenate([self.hiddenLayer.output_left,self.hiddenLayer.output_right], axis=1),
...
if __name__ == '__main__':
    ...
    results = []
#    learningrate=0.03
    for nepochs in range (200,600, 100):    
        for learningrate in numpy.arange(0.025, 0.1, 0.005):     
            for batchsize in range (50,150,25):
                ...
                validation_error = test_mlp(file_list,learning_rate=learningrate,n_epochs=nepochs, batch_size = batchsize, n_hidden=100)
                print(nepochs, learningrate, batchsize, validation_error)
                print("#####################################################")
                results.append([nepochs, learningrate, batchsize, validation_error])  
                
    os.chdir("/home/ambroise/Documents/LSC-Internship/results/Phone_Emb2")            

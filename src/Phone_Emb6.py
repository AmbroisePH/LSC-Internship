# -*- coding: utf-8 -*-
"""
Created on Fri Jul 22 19:50:46 2016

@author: ambroise

GOAL: Phone_Emb2 get phone from the TWO previouses one AND the TWO next ones. Order deos matter (concatanation)
...
def load_data(pathname):
    ...
    train_set_x_left2 = numpy.zeros((n_examples,n_in),dtype='int')
    train_set_x_right2 = numpy.zeros((n_examples,n_in),dtype='int')
    train_set_x_target = numpy.zeros((n_examples,n_in),dtype='int')
    train_set_y = numpy.zeros((n_examples,),dtype='int')
    
    index_vec =   [i for i in range (2,n_examples-2)]
    ...
    for i in index_vec:
        ...
        train_set_x_left2[i,dico[text[i-2]]] =  1
        train_set_x_right2[i,dico[text[i+2]]] = 1
...
#            
    ...
    arr = (train_set_x_left[:len_train_set],
            train_set_x_right[:len_train_set],
            train_set_x_left2[:len_train_set],
            train_set_x_right2[:len_train_set],
            train_set_x_target[:len_train_set],
            train_set_y[:len_train_set])
    train_set = tuple(map(tuple, arr))

    arr = (train_set_x_left[len_train_set+1:len_valid_set],
           train_set_x_right[len_train_set+1:len_valid_set],
            train_set_x_left2[len_train_set+1:len_valid_set],
            train_set_x_right2[len_train_set+1:len_valid_set],
            train_set_x_target[len_train_set+1:len_valid_set],
            train_set_y[len_train_set+1:len_valid_set])
    valid_set = tuple(map(tuple, arr))

    arr = (train_set_x_left[len_valid_set + 1:],
           train_set_x_right[len_valid_set + 1:],
            train_set_x_left2[len_valid_set + 1:],
            train_set_x_right2[len_valid_set + 1:],
            train_set_x_target[len_valid_set + 1:],
            train_set_y[len_valid_set + 1:])
    ...
    def shared_dataset(data_xy, borrow=True):
            ...
            """
            data_x_left, data_x_right, data_x_left2, data_x_right2, data_x_target, data_y = data_xy
            ...
            shared_x_left2 = theano.shared(numpy.asarray(data_x_left2,
                                                   dtype=theano.config.floatX),
                                     borrow=borrow)
                                     
            shared_x_right2 = theano.shared(numpy.asarray(data_x_right2,
                                                   dtype=theano.config.floatX),
                                     borrow=borrow)
            ...
            return shared_x_left, shared_x_right, shared_x_left2, shared_x_right2, shared_x_target, T.cast(shared_y, 'int32')
    
    train_set_x_left, train_set_x_right, train_set_x_left2, train_set_x_right2, train_set_x_target, train_set_y = shared_dataset(train_set)
    valid_set_x_left, valid_set_x_right, valid_set_x_left2, valid_set_x_right2, valid_set_x_target, valid_set_y = shared_dataset(valid_set)
    test_set_x_left, test_set_x_right, test_set_x_left2, test_set_x_right2, test_set_x_target, test_set_y = shared_dataset(test_set)
...
#    print(test_set_y.eval().shape)
    ...
    rval = [(train_set_x_left, train_set_x_right, train_set_x_left2, train_set_x_right2, train_set_x_target, train_set_y),
            (valid_set_x_left, valid_set_x_right, valid_set_x_left2, valid_set_x_right2, valid_set_x_target, valid_set_y),
              (test_set_x_left, test_set_x_right, test_set_x_left2, test_set_x_right2, test_set_x_target, test_set_y)]
    return rval
...
class MLP(object):
    ...
    def __init__(self, rng, input, n_in, n_hidden, n_out):
        ...
        self.hiddenLayer_left2 = HiddenLayer(
            rng=rng,
            input=input[2],
            n_in=n_in,
            n_out=n_hidden,
        ...
        self.hiddenLayer_right2 = HiddenLayer(
            rng=rng,
            input=input[3],
            n_in=n_in,
            n_out=n_hidden,
        ...
        self.hiddenLayer_2 = HiddenLayer(
            rng=rng,
            input=T.concatenate([self.hiddenLayer_left2.output,self.hiddenLayer_left.output,self.hiddenLayer_right.output,self.hiddenLayer_right2.output], axis=1),
            n_in=4*n_hidden,
            n_out=n_hidden, 
            activation=T.tanh
        )
        ...
        self.L1 = (
            ...
            + abs(self.hiddenLayer_left2.W).sum()
            + abs(self.hiddenLayer_right2.W).sum()
        ...
        self.L2_sqr = (
            ...
            + (self.hiddenLayer_left2.W ** 2).sum()
            + (self.hiddenLayer_right2.W ** 2).sum()
...
def test_mlp(pathname,learning_rate=0.01, L1_reg=0.00, L2_reg=0.0001, n_epochs=1000,
   ...
   """
    ...
    train_set_x_left, train_set_x_right, train_set_x_left2, train_set_x_right2, train_set_x_target, train_set_y = datasets[0]
    valid_set_x_left, valid_set_x_right, valid_set_x_left2, valid_set_x_right2, valid_set_x_target, valid_set_y = datasets[1]
    test_set_x_left, test_set_x_right, test_set_x_left2, test_set_x_right2, test_set_x_target, test_set_y = datasets[2]
#
...
#    print(test_set_y.eval().shape)
    ...
    x_left2 = T.matrix('x_left2')  # the data is presented as rasterized images
    x_right2 = T.matrix('x_right2')  # the data is presented as rasterized images
    ...
    classifier = MLP(
        rng=rng,
        input=(x_left, x_right, x_left2, x_right2, x_target),
        n_in=n_in,
        n_hidden=n_hidden,
        n_out=n_in
    )
    ...
    test_model = theano.function(
        ...
        givens={
            ...
            x_left2: test_set_x_left2[index * batch_size:(index + 1) * batch_size],
            x_right2: test_set_x_right2[index * batch_size:(index + 1) * batch_size],
    ...
    validate_model = theano.function(
        ...
        givens={
            ...
            x_left2: valid_set_x_left2[index * batch_size:(index + 1) * batch_size],
            x_right2: valid_set_x_right2[index * batch_size:(index + 1) * batch_size],
    ...
    train_model = theano.function(
        ...
        givens={
            ...
            x_left2: train_set_x_left2[index * batch_size:(index + 1) * batch_size],
            x_right2: train_set_x_right2[index * batch_size:(index + 1) * batch_size],
...
#                         pickle.dump(classifier.logRegressionLayer, f)
                    os.chdir("/home/ambroise/Documents/LSC-Internship/results/Phone_Emb6/Dictio")     
                    
                    SavedModel_name = ('BestModelEmb6_%.2f_%i_%i_%i.pkl' % (learning_rate, n_epochs, batch_size, n_hidden))
